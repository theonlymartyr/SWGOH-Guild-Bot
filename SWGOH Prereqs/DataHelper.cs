    ...
    class DataHelper
    {
        ...
        public TWGuild newGuildStats(PlayerParse.Player guild, string[] toons)
        {
            ...
            try
            {
                if (ignored != null)
                {
                    ...
                    {
                        ignored.Add(s);
                        Console.WriteLine(s + " added tp guild Obj");
                    }
                ...
                foreach (PlayerParse.PlayerElement pe in guild.PlayerList)
                {
                    if (ignore != null)
                    {
                        foreach (string s in ignore)
                        {
                            ...
                            {
                                Console.WriteLine($"{pe.AllyCode} not ignored");
        ...
        public void logCommandInformation(String result)
        {
            ...
            if (ctx.Guild is null)
                File.AppendAllText($@"C:\Users\jake\Documents\swgoh\SWGOH Prereqs\SWGOH Prereqs\Logs\{ctx.User}.txt", message);
            else
                File.AppendAllText($@"C:\Users\jake\Documents\swgoh\SWGOH Prereqs\SWGOH Prereqs\Logs\{ctx.Guild.Name}.txt", message);
        }
        public void logCommandInfo(String info)
        {
            if (ctx.Guild is null)
                File.AppendAllText($@"C:\Users\jake\Documents\swgoh\SWGOH Prereqs\SWGOH Prereqs\Logs\{ctx.User}.txt", DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss") + " :: " + info + " \n");
            else
                File.AppendAllText($@"C:\Users\jake\Documents\swgoh\SWGOH Prereqs\SWGOH Prereqs\Logs\{ctx.Guild.Name}.txt", DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss") + " :: " + info + " \n");
        }
        ...
        public void guildChar(PlayerParse.Player player, CommandContext ctx, DiscordEmbedBuilder embed, string toon)
        {
            ...
            foreach (PlayerParse.PlayerElement pe in player.PlayerList)
            {
                ...
                foreach (PlayerParse.Roster r in pe.Roster)
                {
                    ...
                    {
                        if (r.CombatType == 2) { gt.isShip = true; }
            ...
            foreach (var p in ordered)
            {
                if (i % 10 == 0 && i != 0)
                {
                    playerStrings[(i / 10) - 1] = playerStringer;
                    playerStringer = "";
                }
                if (!gt.isShip)
        ...
        public async void exceptionHandler(Exception e, CommandContext ctx, DiscordEmbedBuilder embed)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        logCommandInfo($"Unknown issues: {e.Message} ");
